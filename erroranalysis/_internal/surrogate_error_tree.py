...
def compute_error_tree(analyzer,
    ...
    if not is_spark(filtered_df):
        booster, filtered_indexed_df, cat_info, accuracy_surrogate = get_surrogate_booster_local(
    ...
    #Total error of (non-surrogate) model
    total_error = filtered_indexed_df.loc[filtered_indexed_df['diff'] == True].shape[0]

    #index_diff    
    index_diff = filtered_indexed_df.loc[filtered_indexed_df['diff'] == True].index.values.tolist()

    #print("Number of Instances : " + str(filtered_indexed_df.shape[0]))
    ...
    return tree, total_error, index_diff, accuracy_surrogate
...
def get_surrogate_booster_local(filtered_df, analyzer, is_model_analyzer,
    ...
    #print("Number of Input Instances : " + str(true_y.shape[0]))
    ...
    surrogate, accuracy_surrogate = create_surrogate_model(analyzer,
    ...
    return surrogate._Booster, filtered_indexed_df, categorical_info, accuracy_surrogate
...
def create_surrogate_model(analyzer,
    ...
    #print('Accuracy of surrogate model : ' + str('{:.2f}'.format(surrogate.score(dataset_sub_features, diff))))
    acc_surrogate = round(surrogate.score(dataset_sub_features, diff),2)

    return surrogate, acc_surrogate
